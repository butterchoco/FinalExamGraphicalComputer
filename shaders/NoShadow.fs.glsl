...
void main()
{
	...
	float spotLightIntensity = 0.8 + 0.6 * max(dot(fNorm, toSpotLightNormal), 0.0);

	gl_FragColor = vec4(meshColor.rgb * lightIntensity + meshColor.rgb * spotLightIntensity, meshColor.a);
}